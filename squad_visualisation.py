...
def prepare_json(away,home):
    """Prepare jsons in format that will be easier to digest for d3.js"""
    ...
    for var in json_data_home:
        var['Players'] = [value for key, value in var.items() if key.startswith('Player_data')]
    return json_data_away, json_data_home, home, away

away, home = get_match_squad('Manchester United')
json_data_away, json_data_home, processsed_home, processed_away = prepare_json(away, home)

all_matches = pd.concat([processsed_home, processed_away])
...
for column in all_matches.columns:
    if 'Player_data' in column:
        count_list.append(pd.value_counts(all_matches[column]))
